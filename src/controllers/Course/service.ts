...
class CourseService {
  ...
  public static async getAll(req: Request) {
    ...
    const data = await Course.find(filterObject).sort(orderObject)
  ...
  public static async getOne(query: any) {
    let data = null
    if (query && query.id) {
      data = await Course.findById(query.id)
    } else {
      data = await Course.findOne(query)
    }
    if (!data) {
   ...
   * @param formData
   */
  public static async create(formData: CourseAttributes) {
    const value = useValidation(courseSchema.create, formData)
   ...
   * @param formData
   */
  public static async update(id: string, formData: CourseAttributes) {
    const data = await this.getOne(id)

    const value = useValidation(courseSchema.create, {
      ...data.toJSON(),
      ...formData,
    })
